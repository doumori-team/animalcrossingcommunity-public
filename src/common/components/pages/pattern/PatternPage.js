...
const PatternPage = () =>
{
	...
	const orgPalette = [...palettes[pattern.paletteId-1].colors];
	...
	for (var key in palette)
	{
		...
		{
			indexes[key] = (colors.indexOf(palette[key]))+1;
		}
		else
		{
			indexes[key] = i;
	...
	const toggleColorFunc = (rgb, index) =>
	{
								...
								<div className='PatternPage_palette'>
									<div className='PatternPage_palettes'>
										{palette.map((rgb, index) => {
											...
											{
												let patternColor = colorInfo.find(c => c.hex === rgb);
