...
const ListingPage = () =>
{
	const {listing, characters, game, towns, friendCodes, userEmojiSettings,
		currentUserEmojiSettings} = useLoaderData();
	...
	if (listing.game && listing.game.id > constants.gameIds.ACGC)
	{
	...
	return (
		<div className='ListingPage'>
			<RequirePermission permission='use-trading-post'>
											...
											{filteredCharacters.length > 0 ? (
												...
												towns.length === 0 ? (
													<>
														...
														<Link to={`/profile/${encodeURIComponent(currentUser.id)}/towns/add`}>{'Click here'}</Link>
														{' to setup a town.'}
													</>
												) : (
													characters.length > 0 ? (
														<>
															...
															<Link to={`/profile/${encodeURIComponent(currentUser.id)}/friend-codes`}>{'Click here'}</Link>
															{' to update your friend codes.'}
														</>
													) : (
														<>
															{'You have no characters set up. '}
															<Link to={`/profile/${encodeURIComponent(currentUser.id)}/characters/add`}>{'Click here'}</Link>
										...
										{listing.game.id === constants.gameIds.ACGC ? (
											filteredCharacters.length > 0 && (
												<div className='ListingPage_secretCode'>
													...
													{listing.offers.accepted.character && (
														<RequireUser id={listing.creator.id} silent>
															<h3 className='ListingPage_acceptedCharacter'>
				...
				{listing.status === constants.tradingPost.listingStatuses.closed && (
