...
const TradingPostPage = () =>
{
	const {listings, totalCount, page, pageSize, acgames, creator, type,
		...
		bioLocation, comment, gameId, acgameCatalog, initialResidents} = getData(useAsyncValue());

	const [selectedGameId, setSelectedGameId] = useState(gameId);
	const [residents, setResidents] = useState(initialResidents && gameId > 0 ? initialResidents[gameId] : []);

	const link = `&creator=${encodeURIComponent(creator)}
		&type=${encodeURIComponent(type)}
		&gameId=${encodeURIComponent(selectedGameId)}
		&bells=${encodeURIComponent(bells)}
	...
	const changeGame = (event) =>
	{
		...
		setSelectedGameId(isNaN(gameId) ? '' : Number(event.target.value));
		setResidents(initialResidents && gameId > 0 ? initialResidents[gameId] : []);
	}

	const handleItemsLookup = async (query) =>
	{
		...
		<div className='TradingPostPage'>
			<RequirePermission permission='use-trading-post'>
				...
				<Search callback='/trading-post'>
					...
					{selectedGameId > constants.gameIds.ACGC && (
						<>
						<Form.Group>
							<Select
								name='villagers'
...
export async function loadData(_, {page, creator, type, gameId, bells, items, villagers, active, wishlist, bioLocation, comment})
{
	return Promise.all([
...
{
	const [acgames, returnValue, acgameCatalog, residents, acItemsCatalog] = data;
