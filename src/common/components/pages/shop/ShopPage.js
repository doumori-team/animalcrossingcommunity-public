...
import { Alert, Form, RichTextArea, Select, Text } from '@form';
...
const ShopPage = () =>
{
	const {shop, shopServices, currentUserEmojiSettings, shopRoles,
		acGameCatalogs, acgames} = getData(useAsyncValue());
			...
			return (
				...
				{shopServices.map(service =>
					service.default ? (
						...
						<li><ReportProblem type={constants.userTicket.types.shopServiceName} id={service.id} />{service.name}: <ReportProblem type={constants.userTicket.types.shopServiceDescription} id={service.id} />{service.description}</li>
					)
				)}
				</ul>
	...
	const getApplySection = () =>
	{
		...
		else
		{
			return (
				...
				{shopRoles.map(role =>
					<li><ReportProblem type={constants.userTicket.types.shopRoleName} id={role.id} />{role.name}: <ReportProblem type={constants.userTicket.types.shopRoleDescription} id={role.id} />{role.description} (Available: {role.positionsAvailable})</li>
				)}
				</ul>
			...
			{shop.statData.length > 0 && (
				...
				<RequireLargeScreen size='657'>
					...
					>
						...
						<Legend />
						{shop.games.map(g =>
							<Bar dataKey={g.shortname} fill={g.color} />
						)}
					</BarChart>
				</RequireLargeScreen>
				</>
				...
				{shop.extraStatData.stats.length > 0 && (
					...
					<RequireLargeScreen size='657'>
						...
						>
							...
							<Legend />
							{shopServices.map(s =>
								<Bar dataKey={s.name} stackId='a' fill={utils.getRandomColor()} />
							)}
						</BarChart>
					</RequireLargeScreen>
					</>
