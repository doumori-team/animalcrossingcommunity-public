...
const UserTicketPage = () =>
{
	const {userTicket, denyReasons, rules, actions, boards, ticketUserEmojiSettings,
		banLengths, userEmojiSettings, currentUserEmojiSettings} = useLoaderData();

	const [action, setAction] = useState('no_action');
	const [rule, setRule] = useState(rules.find(c => c !== undefined).rules.find(r => r !== undefined));
	const [actionId, setActionId] = useState(actions.find(a => a.identifier === 'no_action').id);
	...
	const changeAction = (event) =>
	{
		...
		setAction(actions.find(a => a.id === actionId).identifier);
				...
				<UserContext.Consumer>
					{user => user && (
						...
						<div className='UserTicketPage_info'>
							...
							{userTicket.reportedUsers.length > 1 && (
								<div className='UserTicketPage_submitted'>
									All Submitters
									<ul>
											...
											>
												...
												<Form.Group>
													<Select
														...
														optionsMapping={{
															value: 'id',
															label: (rule) => `${rule.categoryId}.${rule.number} - ${rule.name ? rule.name : rule.description}`,
														}}
														required
														useReactSelect
													/>
