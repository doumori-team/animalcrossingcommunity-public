...
import { characterShape, characterGameShape, townShape } from '@propTypes';
import { Form, Check, Text } from '@form';
import { EditKeyboard, Keyboard } from '@layout';

const EditCharacter = ({character, town, gameInfo}) =>
{
	const {info, houseSizes, bedLocations, faces} = gameInfo;
				...
				<div className='EditCharacter_section'>
					<Form.Group>
						<Text
							...
							name='bells'
							label='Bells'
							value={character ? character.bells : 0}
						...
						<>
							<Form.Group>
								<Text
									...
									name='nookMiles'
									label='Nook Miles'
									value={character ? character.nookMiles : 0}
									max={constants.max.number}
								/>
							</Form.Group>
					...
					<Form.Group>
						<Text
							...
							name='debt'
							label='Nook Debt'
							value={character ? character.debt : 0}
							max={constants.max.number}
						/>
					</Form.Group>
					...
					<Form.Group>
						<Text
							type='number'
							name='hraScore'
							value={character ? character.hraScore : 0}
