...
async function adopt({adopteeId, scoutId})
{
	if (!this.userId)
	{
	...
	{
		const permGranted = await this.query('v1/permission', {permission: 'adoption-reassign'});
	...
	// Check parameters
	const adoptee = await this.query('v1/user', {id: adopteeId});

	if (typeof(adoptee) === 'undefined' || adoptee.length === 0)
	{
	...
	if (scoutId)
	{
		...
		if (typeof(scout) === 'undefined' || scout.length === 0)
		{
			throw new UserError('no-such-user');
		}
	...
	const nodeId = await db.transaction(async query =>
	{
		if (scoutId)
		{
			// Lock old thread
