...
async function contact({id, text, format})
{
	...
	const contacts = await db.query(`
		...
		WHERE shop_role.shop_id = $1::int AND shop_role.contact = true AND shop_role.active = true
	`, shop.id);
	...
	const threadId = await db.transaction(async query =>
	{
		...
		await Promise.all([
			(contacts.length > 0 ? contacts.map(c => c.user_id) : shop.owners.map(o => o.id)).concat([this.userId]).map(async (userId) => {
				await query(`
