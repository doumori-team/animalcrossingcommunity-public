...
async function threads({page, shopId, category, type, status, waitlisted, locked})
{
		...
		db.query(`
			SELECT
				...
				EXISTS (
					...
					FROM shop_user
					WHERE shop_user.shop_id = shop.id AND shop_user.user_id = $1 AND shop_user.active = true
				) AS employee
			FROM shop
			LEFT JOIN shop_user ON (shop_user.shop_id = shop.id AND shop_user.user_id = $1 AND shop_user.active = true)
			LEFT JOIN shop_order ON (shop_order.shop_id = shop.id AND shop_order.customer_id = $1)
			LEFT JOIN shop_application ON (shop_application.shop_id = shop.id AND shop_application.user_id = $1)
			LEFT JOIN shop_node ON (shop_node.shop_id = shop.id)
			LEFT JOIN user_node_permission ON (user_node_permission.node_id = shop_node.node_id AND user_node_permission.user_id = $1 AND user_node_permission.granted = true)
			WHERE shop_user.user_id IS NOT NULL OR shop_order.customer_id IS NOT NULL OR shop_application.user_id IS NOT NULL OR user_node_permission.user_id IS NOT NULL
