...
async function save({id, name, shortDescription, description, format, fee, games,
	perOrders, stackOrQuantities, completeOrders, items, vacationStartDate,
	vacationEndDate, allowTransfer, active, fileId})
{
	...
	const shopId = await db.transaction(async query =>
	{
		if (id > 0)
		{
