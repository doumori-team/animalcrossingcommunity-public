...
async function order({id, gameId, serviceId, items, quantities, comment})
{
	const permissionGranted = await this.query('v1/permission', {permission: 'order-apply-shops'});
	...
	if (items.length > 0)
	{
		const catalogItems = (await ACCCache.get(`${constants.cacheKeys.sortedAcGameCategories}_${gameId}_all_items`)).filter(item => item.tradeable && !game.items.includes(item.id));
	...
	const orderId = await db.transaction(async query =>
	{
		let order;
