...
async function thread({id, category})
{
	...
	if (category === constants.shops.categories.orders)
	{
		...
		const catalogItems = await ACCCache.get(`${constants.cacheKeys.sortedAcGameCategories}_${thread.game_id}_all_items`);

		await this.query('v1/notification/destroy', {id: thread.id, type: constants.notification.types.shopOrder});
	...
	else if (category === constants.shops.categories.threads)
	{
		...
		const [[thread], [latestPage], [latestPost]] = await Promise.all([
			...
			`, id, this.userId, constants.threadPageSize),
			db.query(`
				SELECT
					(
						SELECT node.id
