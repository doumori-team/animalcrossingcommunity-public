...
async function create({orderId, applicationId, shopId, title, users, text, format})
{
	...
	if (orderId)
	{
		const order = await this.query('v1/shop/thread', {id: orderId, category: constants.shops.categories.orders});
	...
	{
		const application = await this.query('v1/shop/thread', {id: applicationId, category: constants.shops.categories.applications});
