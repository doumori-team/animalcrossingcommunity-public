...
import { APIThisType, ShopNodeShopType, ShopType, ThreadOrderType, ThreadApplicationType } from '@types';
...
async function shop(this: APIThisType, { id }: shopProps): Promise<ShopNodeShopType>
{
	...
	const [shop, customers, order, application]: [ShopType, { user_id: number }[], ThreadOrderType | null, ThreadApplicationType | null] = await Promise.all([
	...
	return <ShopNodeShopType>{
		id: shop.id,
		name: shop.name,
		customerIds: customers.map(c => c.user_id),
