...
async function town({id})
{
	...
	return {
		...
		tune: town.town_tune_name ? {
			...
			creator: tuneCreator,
			notes: town.town_tune_notes.match(/.{4}/g).map(hex => parseInt(hex, 16))
		} : null,
		museum: museum,
...
async function getNativeFruit(id)
{
...
async function getPublicWorkProjects(id, gamePWPs)
{
...
async function getResidents(id, residents)
{
...
async function getIsland(id, residents)
{
...
async function getMapTiles(id)
{
	...
	{
		return hexMapTiles.map_tiles.match(/.{4}/g).map(hex => parseInt(hex, 16));
	}

	return [];
}

async function getMuseum(id, gameId)
{
	let museum = [];
...
async function getMapDesignData(id, gameId)
{
	...
	{
		mapDesignData['colorData'] = colorData.color_data.match(/.{4}/g).map(hex => colors[parseInt(hex, 16)]);
	}
	...
	if (cursorData.cursor_data)
	{
		...
		mapDesignData['cursorData'] = cursorData.cursor_data.match(/.{4}/g).map(pos => {
			return rectTypes[parseInt(pos, 16)].value;
		});
	}
	...
	{
		mapDesignData['flipData'] = flipData.flip_data.match(/.{4}/g).map(hex => colors[parseInt(hex, 16)]);
	}
	...
	{
		mapDesignData['imageData'] = imageData.image_data.match(/.{4}/g).map(hex => images[parseInt(hex, 16)] ? images[parseInt(hex, 16)] : String(parseInt(hex, 16)));
	}
