...
async function create({id, type})
{
	...
	)
	{
		...
		if (type === types.PT)
		{
			...
			`, nodeId, constants.nodePermissions.read))
				.filter(u => u.user_id !== childNode.user_id)
				.map(u => u.user_id);
		}
		else if ([types.FT, types.FB].includes(type))
		{
		...
		else if (type === types.usernameTag)
		{
			...
			userIds = (await Promise.all(usernames.map(async (username) =>
			{
	...
	{
		let listingId = referenceId, offer, listingComment;
		...
		const listing = await this.query('v1/trading_post/listing', {id: listingId});
		...
		else if (type === types.scoutBT)
		{
			...
			userIds = userIds.concat(
				...
				`, constants.staffIdentifiers.scout))
					.filter(u => u.id !== childNode.user_id)
					.map(u => u.id)
			);
		}
	}
	...
	)
	{
		...
		if (type === types.modminUT)
		{
			...
			`, constants.staffIdentifiers.mod))
				.filter(u => u.id !== userTicket.assignee_id)
				.map(u => u.id);
	...
	)
	{
		...
		if (type == types.feature)
		{
			...
			`, [
				...
				.filter(u => u.id !== feature.created_user_id)
				.map(u => u.id);
		}
		else
		{
	...
	if (type === types.announcement)
	{
		...
		await Promise.all(emailUsers.map(async (emailUser) =>
		{
			try
			{
	...
	for (let i = 0; i < userIds.length; i += chunkSize)
	{
		...
		`, userIds.slice(i, i + chunkSize), useReferenceId, notificationType.id, description);
	}
	...
	await Promise.all(accountSettings.map(async (user) =>
	{
...
function getEmailText(notification, userId)
{
	const vbnewline = '<br/>';
