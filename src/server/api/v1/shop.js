...
async function shop({id})
{
	...
	let statData = [], extraStatData = null;

	if (shopServicesData.length > 0 || shopDefaultServicesData.length > 0)
	{
		shopDefaultServicesData.concat(shopServicesData).map(s => {
			const stat = statData.find(x => x.name === s.name)
	...
	if (statsUsers.find(u => u.id === this.userId))
	{
		extraStatData = {
			stats: [],
			employees: [],
		...
		shopDefaultServicesUsers.concat(shopServicesUsers).map(s => {
			const stat = extraStatData.stats.find(x => x.username === s.username)
		...
		extraStatData.employees = employees.map(u => {
			return {
				id: u.id,
				username: u.username,
		...
		games: games.length > 0 ?
			games.map(game => {
				...
				return {
					...
					items: items.filter(i => i.game_id === game.id).map(i => i.catalog_item_id),
					color: color,
				};
			})
		: [],
