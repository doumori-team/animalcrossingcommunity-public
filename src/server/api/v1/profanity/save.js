...
async function save({id, word, activate})
{
	const permissionGranted = await this.query('v1/permission', {permission: 'profanity-admin'});
	...
	if (id > 0)
	{
		...
		{
			const permissionGranted = await this.query('v1/permission', {permission: 'activate-profanity-words'});
 ...
 */
async function createThread(word, content, active)
{
