...
async function save({id, title, description, statusId, categoryId, isBug,
	isExploit, staffOnly, readOnly, assignedUsers, staffDescription, format, staffDescriptionFormat})
{
	const permissionGranted = await this.query('v1/permission', {permission: 'suggest-features'});
	...
	if (!Array.isArray(assignedUsers))
	{
		if (assignedUsers)
		{
			...
			assignedUsers = assignedUsers.split(',').map(username => username.trim());
		}
		else
		{
			assignedUsers = [];
	...
	if (id > 0)
	{
		...
		if (managePermission)
		{
			await db.query(`
				UPDATE feature
				SET title = $1::text, description = $2::text, status_id = $3::citext, category_id = $4::int, is_bug = $5::boolean, staff_only = $7, read_only = $8, format = $9, staff_description = $10, staff_description_format = $11
				WHERE id = $6::int
	...
	else
	{
		...
		if ([
			...
			constants.staffIdentifiers.developer,
		...
		[id] = await db.query(`
			...
			VALUES ($1::text, $2::text, $3::citext, $4::int, $5::boolean, $6, $7, $8, $9, $10, $11)
			RETURNING id
