...
async function title(this: APIThisType, {pathname}: titleProps) : Promise<string>
{
    ...
    switch (sections[1])
    {
        ...
        case 'shop':
            const shopPermission:boolean = await this.query('v1/permission', {permission: 'view-shops'});

            if (shopPermission)
            {
                if (sections[2] === 'order' || sections[2] === 'application')
                {
                    return `Shops & Services${ending}`;
                }
