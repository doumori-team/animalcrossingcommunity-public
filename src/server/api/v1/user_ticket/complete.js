...
async function complete({id, ruleId, violationId, actionId, updatedContent, boardId, banLengthId})
{
	...
	const actions = await this.query('v1/user_ticket/actions');
	...
	const currentBan = await this.query('v1/users/ban_length', {id: userTicket.violator_id});
	...
	await Promise.all([
		db.query(`
			...
			WHERE id = $1::int
		`, id, status.id, violationId > 0 ? violationId : null, checkAction.identifier === 'modify' ? updatedContent : null, actionId, ruleId),
...
async function getEmailText(userTicketId, banLengthId)
{
