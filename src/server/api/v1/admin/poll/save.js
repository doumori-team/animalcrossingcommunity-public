...
async function save({id, question, description, startDate, duration,
	isMultipleChoice, isEnabled, options})
{
	const permission = await this.query('v1/permission', {permission: 'polls-admin'});
	...
	// Perform queries
	const pollResult = await db.transaction(async query =>
	{
		if (id > 0)
		{
