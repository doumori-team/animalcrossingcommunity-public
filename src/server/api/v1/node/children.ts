...
import { APIThisType, NodeChildrenType } from '@types';

async function children(this: APIThisType, { id, order, reverse, page, showLocked }: childrenProps): Promise<NodeChildrenType>
{
	return await Sentry.startSpan({ name: 'v1/node/children', attributes: { nodeId: id } }, async () =>
	{
		...
		const [permission, [node], groupIds, ggBoardPerm, publicBoards, archivedBoards, staffBoards] = await Sentry.startSpan({ name: 'children.init' }, () => Promise.all([
			...
			`),
			id === constants.boardIds.staffThreads ? db.query(`
				SELECT id
				FROM node
		...
		let resultsQuery: Promise<any> | null = null;
		...
		{
			resultsQuery = Sentry.startSpan({ name: 'children.query.pts' }, () => db.query(`
				SELECT
					nodes.*,
				...
				FROM (
					SELECT
						...
						$5::int AS parent_node_id,
						$6::int AS parent_node_id2,
						0 AS post_number,
						0 AS page_number,
			...
			`, pageSize, offset, showLocked, this.userId, constants.boardIds.privateThreads, constants.boardIds.accForums));
		}
		...
		{
			resultsQuery = Sentry.startSpan({ name: 'children.query.publicThreads' }, () => db.query(`
				SELECT
					nodes.*,
					last_revision.id AS revision_id,
					last_revision.title,
		...
		// if viewing special All Staff Threads, view permitted staff boards threads
		// (can just do user group perms here, I just kept it the same as All Public Threads query)
		else if (id === constants.boardIds.staffThreads)
		{
			resultsQuery = Sentry.startSpan({ name: 'children.query.staffThreads' }, () => db.query(`
				SELECT
					nodes.*,
					last_revision.id AS revision_id,
					last_revision.title,
			...
			`, staffBoards.map((x: { id: string }) => Number(x.id)), pageSize, offset, constants.nodePermissions.read, groupIds, this.userId));
		}
		else if (archivedBoards.map((x: { id: string }) => Number(x.id)).includes(id))
		{
			resultsQuery = Sentry.startSpan({ name: 'children.query.archived' }, () => db.query(`
				SELECT
					archived_threads.id,
				...
				LEFT JOIN node AS parent ON (parent.id = archived_threads.parent_node_id)
		...
		{
			resultsQuery = Sentry.startSpan({ name: 'children.query.generic' }, () => db.query(`
				WITH thread_posts AS (
					SELECT
						...
						((ROW_NUMBER() OVER (
							PARTITION BY node.parent_node_id
							ORDER BY node.creation_time ASC
						) - 1) / $5) + 1 AS page_number
					FROM node
					JOIN node AS parent ON (parent.id = node.parent_node_id)
					WHERE node.parent_node_id = $3 AND parent.type = 'thread'
				)
				SELECT
					nodes.*,
					last_revision.id AS revision_id,
			...
			`, pageSize, offset, id, showLocked, constants.threadPageSize));
		}

		const [count, childNodes] = await Sentry.startSpan({ name: 'children.getChildren' }, () =>
			db.getChildren(resultsQuery, this.query, this.userId, true),
		);
