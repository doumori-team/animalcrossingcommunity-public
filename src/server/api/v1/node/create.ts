...
import { APIThisType, UserType, UserDonationsType, NodeLiteType, MarkupStyleType } from '@types';
	...
	type, boardId, fileIds, fileNames, fileWidths, fileHeights, fileCaptions}: createProps) : Promise<{id: number}>
{
	...
	if (addUserIds.length > 0 || removeUserIds.length > 0)
	{
		...
		else if (threadDetails != null && [constants.boardIds.privateThreads, constants.boardIds.shopThread].includes(threadDetails?.parentId))
		{
