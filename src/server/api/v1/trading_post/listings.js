...
async function listings({page, creator, type, gameId, bells, items, villagers,
	active, wishlist, bioLocation, comment})
{
	...
	if (bells > 0 || items.length > 0 || villagers.length > 0 || utils.realStringLength(comment) > 0)
	{
		query += `
	...
	if (active > 0 || utils.realStringLength(bioLocation) > 0)
	{
		query += `
	...
	if (wishlist)
	{
		...
		var wishlistItemsSearch = [];

		if (gameId === 0)
		{
			const userCatalogItems = await this.query('v1/users/catalog', {id: this.userId});
			wishlistItemsSearch = userCatalogItems.filter(item => item.isWishlist);
		}
		else if (gameId > 0)
		{
			const characters = await this.query('v1/users/characters', {id: this.userId});

			wishlistItemsSearch = (await Promise.all(characters.filter(c => c.game.id === gameId).map(async(character) => {
				const catalog = await this.query('v1/character/catalog', {id: character.id});
		...
		if (wishlistItemsSearch.length > 0)
		{
			...
			`, wishlistItemsSearch, listingStatuses.open)).map(l => l.id);
