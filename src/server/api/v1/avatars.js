...
export default async function avatars()
{
	...
	const avatarBackgrounds = await ACCCache.get(constants.cacheKeys.alphabeticalAvatarBackgrounds);
	const sortedBellShopItems = await ACCCache.get(constants.cacheKeys.sortedBellShopItems);
	const bellShopCategories = await ACCCache.get(constants.cacheKeys.bellShopCategories);

	const availableBackgrounds = getAvailableAvatars(
		mappedPerms,
		grantedAvatarPerms ? grantedAvatarPerms.background_ids : [],
		mappedBellShopRedeemedItems.length > 0 ? sortedBellShopItems[bellShopCategories.find(c => c.name === constants.bellShop.categories.avatarBackgrounds).id].filter(c => mappedBellShopRedeemedItems.includes(c.id)).map(c => Number(c.internalId)) : [],
	...
	const avatarCharacters = await ACCCache.get(constants.cacheKeys.alphabeticalAvatarCharacters);

	const availableCharacters = getAvailableAvatars(
		mappedPerms,
		grantedAvatarPerms ? grantedAvatarPerms.character_ids : [],
		mappedBellShopRedeemedItems.length > 0 ? sortedBellShopItems[bellShopCategories.find(c => c.name === constants.bellShop.categories.avatarCharacters).id].filter(c => mappedBellShopRedeemedItems.includes(c.id)).map(c => Number(c.internalId)) : [],
	...
	const avatarAccents = await ACCCache.get(constants.cacheKeys.alphabeticalAvatarAccents);

	const availableAccents = getAvailableAvatars(
		mappedPerms,
		grantedAvatarPerms ? grantedAvatarPerms.accent_ids : [],
		mappedBellShopRedeemedItems.length > 0 ? sortedBellShopItems[bellShopCategories.find(c => c.name === constants.bellShop.categories.avatarAccents).id].filter(c => mappedBellShopRedeemedItems.includes(c.id)).map(c => Number(c.internalId)) : [],
	...
	const avatarColorations = await ACCCache.get(constants.cacheKeys.alphabeticalAvatarColorations);

	const availableColorations = getAvailableAvatars(
		mappedPerms,
		grantedAvatarPerms ? grantedAvatarPerms.coloration_ids : [],
		mappedBellShopRedeemedItems.length > 0 ? sortedBellShopItems[bellShopCategories.find(c => c.name === constants.bellShop.categories.backgroundColorations).id].filter(c => mappedBellShopRedeemedItems.includes(c.id)).map(c => Number(c.internalId)) : [],
